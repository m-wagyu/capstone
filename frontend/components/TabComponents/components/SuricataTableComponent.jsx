...
export default function SuricataTableComponent({
  columns,
  data,
  loading,
  page,
  ...
  if (!columns.length)
    return (
      <Typography variant="h5">
        No Columns for SuricataTableComponent
      </Typography>
    );
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                {columns.map((each, i) => {
                  return (
                    <TableCell key={i} {...each.title_props}>
                      {each.title}
                    </TableCell>
                  );
                })}
            ...
            <TableBody>
              {data &&
                noPaginationCheck(data).map((row, key) => {
                  return (
                    <TableRow tabIndex={-1} key={key}>
                      {columns.map((each, i) => {
                        return (
                          <TableCell key={i} {...each.data_row_props}>
                            {row[each.data_row_name]}
                          </TableCell>
                        );
                      })}
                    </TableRow>
